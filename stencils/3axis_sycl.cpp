...
void printInfo(cl::sycl::device &Device) {
  std::cout << "Using OpenCL " << (Device.is_cpu() ? "CPU" : "GPU")
            ...
            << "} from {" << Device.get_info<cl::sycl::info::device::vendor>()
            << "}" << std::endl;
...
void d3pt7() {
  ...
  cl::sycl::queue squeue(*sycl_device,
                         {cl::sycl::property::queue::enable_profiling()});

  copyToBrick<3>({STRIDEG, STRIDEG, STRIDEG}, {PADDING, PADDING, PADDING},
                 {0, 0, 0}, in_ptr, grid_ptr, bIn);
  ...
  auto arr_func = [&arr_in, &arr_out]() -> void {
    _TILEFOR arr_out[k][j][i] =
        coeff[5] * arr_in[k + 1][j][i] + coeff[6] * arr_in[k - 1][j][i] +
  ...
  nd_range<1> nworkitem(range<1>(1024 * SYCL_SUBGROUP),
                        range<1>(SYCL_SUBGROUP));
...

  {
    ...
    ed_event.wait();
    double elapsed =
        ed_event.get_profiling_info<info::event_profiling::command_end>() -
        ed_event.get_profiling_info<info::event_profiling::command_start>();
    elapsed *= 1e-9;
    std::cout << "Brick: " << elapsed << std::endl;

    if (!compareBrick<3>({N, N, N}, {PADDING, PADDING, PADDING}, {GZ, GZ, GZ},
                         out_ptr, grid_ptr, bOut)) {
            ...
            for (long k = 0; k < TILE; ++k)
              for (long j = 0; j < TILE; ++j)
                for (long i = 0; i < TILE; ++i) {
                  auto aval =
                      arr_out[tk * TILE + k + PADDING][tj * TILE + j + PADDING]
                             [ti * TILE + i + PADDING];
